...
class ErrorEstimationManager : public rclcpp_lifecycle::LifecycleNode {
public:
  explicit ErrorEstimationManager(const rclcpp::NodeOptions &options = rclcpp::NodeOptions());

  ~ErrorEstimationManager() override;
...
private:
  ...
  /* EKF //{ */
  std::unique_ptr<laser_uav_estimators::ErrorStateEstimator> es_ekf_;
  /*//}*/
  ...
  bool is_initialized_es_ekf_{false};
  ...
  laser_uav_estimators::NoiseGains                      _process_noise_gains_;
  laser_uav_estimators::ProcessNoiseGains               _measurement_noise_gains_;
  std::map<int, laser_uav_estimators::InnovationLimits> _limits_;
